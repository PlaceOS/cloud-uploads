...
describe('Helper functions', () => {
    describe('humanReadableByteCount', () => {
        test('should parse byte values', () => {
            expect(humanReadableByteCount(64 * 1024)).toBe('64.0 KB');
            expect(humanReadableByteCount(128 * 1024 * 1024)).toBe('128.0 MB');
            expect(humanReadableByteCount(32.5 * 1024 * 1024 * 1024)).toBe(
                '32.5 GB',
            );
        });
        test('should parse byte values with SI units', () => {
            expect(humanReadableByteCount(64 * 1000, true)).toBe('64.0 kiB');
            expect(humanReadableByteCount(128 * 1000 * 1000, true)).toBe(
                '128.0 MiB',
            );
            expect(
                humanReadableByteCount(32.5 * 1000 * 1000 * 1000, true),
            ).toBe('32.5 GiB');
        });
    });

    describe('hexToBinary', () => {
        test('should convert hex into binary', () => {
